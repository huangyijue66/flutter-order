    ...
    return table.listTables().length == 0
      ...
      child: CircularProgressIndicator(),
        ...
        itemBuilder: (context, index){
          return Container(
              child:ListTile(
                leading: Icon(Icons.restaurant_menu,color: Colors.blue,size: 75,),
                title: Text('${index + 1}号桌',style: TextStyle(color: Colors.red,fontSize: 40),),
                subtitle: Text('${table.listTables()[index].seats}人桌',style: TextStyle(color: Colors.blue,fontSize: 20),),
                trailing: table.listTables()[index].sitable == false ?
                Text('不可入座') :
                RaisedButton(
                    color: Colors.orange,
                    padding: EdgeInsets.all(5),
                    child: Text('入座',style: TextStyle(fontSize: 30),),
                    onPressed:
                        waiterModel.getIsLogin() == true
                        ?(){
                          table.sit(index);
                          table.setCurrentTable(table.listTables()[index]);
                          Navigator.of(context).pushNamed('/kind');}
                        : (){
  ...
  List<Widget> _getListTiles(BuildContext context) {
    return [
      ListTile(
        ...
        title: Text('没跳转',style: TextStyle(color: Colors.red),),
        onTap: () {
      ...
      ListTile(
        ...
        trailing: Icon(Icons.chevron_right),
        title: Text('点也没用',style: TextStyle(color: Colors.red),),
        onTap: () {
          Navigator.pop(context);
        },
      ),
      ListTile(
        ...
        trailing: Icon(Icons.chevron_right),
        title: Text('点了回主页',style: TextStyle(color: Colors.red),),
        onTap: () {
          Navigator.pop(context);
        },
      ),
